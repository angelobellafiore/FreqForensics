...
if __name__ == '__main__':
    ...
    # val_transform must be deterministic — same input, same output every time
    ...
    # train_transform may differ between runs due to random augmentations
    tensor_train2 = train_transform(dummy_crop)
    # Not guaranteed to differ (very small probability of identical random params)
    # but we verify it at least produces a valid tensor
    assert tensor_train2.shape == (3, 224, 224)
    print("  train_transform second run: OK")

    # Verify normalisation: after Normalize, values are NOT in [0,1] anymore —
    # they are centred around 0 with std ~1. This is expected and correct.
